"""
Momentum NASDAQ Agent - Technical Trend Following

Self-contained agent implementing momentum strategy for NASDAQ 100 trading.
Uses shared ClaudeCodeAgent base with momentum-specific methodology.
"""
...
from agent.momentum_agent.strategy import MomentumStrategy
from agent.claude_code_agent.shared.instructions import build_claude_md
from prompts.agent_prompt import all_nasdaq_100_symbols


class MomentumNASDAQAgent(ClaudeCodeAgent):
    """Momentum trend-following agent with conservative risk management."""
    ...
    def _setup_claude_instructions(self) -> None:
        """Generate CLAUDE.md from shared base + momentum-specific sections."""
    ...
    def _get_mcp_config(self) -> dict:
        """MCP server configuration for momentum strategy."""
        return {
            "mcpServers": {
                "sequential-thinking": {
                    "command": "npx",
    ...
    def __str__(self) -> str:
        return f"MomentumNASDAQAgent(signature='{self.signature}')"
