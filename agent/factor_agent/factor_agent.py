"""
Factor NASDAQ Agent - Academic Multi-Factor Investing for NASDAQ 100

Self-contained agent implementing factor strategy for NASDAQ 100 trading.
Uses shared ClaudeCodeAgent base with factor-specific methodology.
"""
...
from agent.factor_agent.strategy import FactorStrategy
from agent.claude_code_agent.shared.instructions import build_claude_md
from prompts.agent_prompt import all_nasdaq_100_symbols


class FactorNASDAQAgent(ClaudeCodeAgent):
    """Factor agent with balanced risk management."""

    def __init__(self, signature: Optional[str] = None, **kwargs):
        self.strategy = FactorStrategy()
    ...
    def _setup_claude_instructions(self) -> None:
        """Generate CLAUDE.md from shared base + factor-specific sections."""
    ...
    def _get_mcp_config(self) -> dict:
        """MCP server configuration for factor strategy."""
        return {
            "mcpServers": {
                "sequential-thinking": {
                    "command": "npx",
    ...
    def __str__(self) -> str:
        return f"FactorNASDAQAgent(signature='{self.signature}')"
